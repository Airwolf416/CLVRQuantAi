...
- **Statistical Brain on the morning email brief:** `server/dailyBrief.ts` `generateBriefContent()` calls `getBrainSummary()` (fail-open) and prepends a top-6 PREFERRED + top-6 SUPPRESSED combos block right after the LIVE MARKET DATA section so `topTrade` and `additionalTrades` favour empirically-winning combos and avoid bleeders. Verdict thresholds are mirrored locally (`SUPPRESS_WR=0.25`, `PREFERRED_WR=0.60`, `MIN_SAMPLE_SUPP=15`, `MIN_SAMPLE_PREF=20`) since `ComboStat` doesn't carry a `verdict` field — that's derived per-call by `getBrainFor`.
- **Trade Ideas asset-class selector:** `TopTradeIdeas.jsx` adds an ASSET CLASS toggle (All / Crypto / Equities / Commodities / Forex) rendered ABOVE the existing PERP/SPOT/BOTH selector — picked first to scope the run before choosing market type. `assetClass` flows into `buildMarketSnapshot` which drops rows from non-matching classes in Sections A/B/C, gates the per-class placeholder lines, filters `liveSignals` to only tokens belonging to the selected class (no foreign-class signal leak in scoped runs), and adds an `ASSET CLASS FILTER: X-ONLY` directive to the filter header instructing the AI to skip the run rather than substitute a different class. FOREX has no perp market on Hyperliquid, so picking FOREX auto-flips active PERP→SPOT and disables the PERP button. ALL behavior is functionally identical to the pre-feature snapshot.
