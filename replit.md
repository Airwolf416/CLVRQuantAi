...
**Symptom:** user selected PERP in AI tab → CLVR AI → Trade Ideas, AI returned an AMD entry priced from Yahoo spot. AMD has no Hyperliquid perp listing.

**Root cause:** `client/src/utils/marketDataSnapshot.js` (`buildMarketSnapshot`) injected ALL three sections regardless of `marketTypeFilter` — A (HL perp), B (HL spot), C (Yahoo/CoinGecko/FMP spot). The AI's only constraint was a prompt rule "must set marketType:PERP". When asked for a PERP idea on equities, the model picked AMD from Section C and labeled it PERP. Same class of leak existed in reverse for SPOT mode (could fabricate funding/OI talking points from Section A). Separately, the full asset universe was always injected — including dozens of neutral / non-moving assets — so the AI often anchored on stale prices instead of actual movers.
