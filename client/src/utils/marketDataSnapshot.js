...
export function buildMarketSnapshot({
  ...
  if (includePerps) {
    ...
    const cryptoPerpRows    = CRYPTO_SYMS.map(s => fmtHLPerp(s, "crypto")).filter(Boolean);
    const equityPerpRows    = EQUITY_SYMS.map(s => fmtHLPerp(s, "equity")).filter(Boolean);
    const commodityPerpRows = METALS_SYMS.map(s => fmtHLPerp(s, "commodity")).filter(Boolean);

    const cryptoLine    = cryptoPerpRows.length    ? `CRYPTO PERPS: ${cryptoPerpRows.join(" | ")}`             : (signalFilter ? "CRYPTO PERPS: (no pump/dump signals — all assets within neutral range)" : "");
    const equityLine    = equityPerpRows.length    ? `EQUITY PERPS (HL synthetic): ${equityPerpRows.join(" | ")}` : "";
    const commodityLine = commodityPerpRows.length ? `COMMODITY PERPS (HL synthetic): ${commodityPerpRows.join(" | ")}` : "";
...
${hlSpotRows.length ? hlSpotRows.join(" | ") : (signalFilter ? "(no pump/dump signals on tracked spot pairs)" : "HL spot data loading")}`;
  ...
  if (includeSpot) {
    ...
    const cryptoCRows = CRYPTO_SYMS.map(s => tagged(s, cryptoPrices, "crypto", s)).filter(Boolean);
    const equityRows  = EQUITY_SYMS.map(s => tagged(s, equityPrices, "equity", s)).filter(Boolean);
    const metalRows   = METALS_SYMS.map(s => tagged(s, metalPrices, "commodity", METAL_LABELS[s] || s)).filter(Boolean);
    const fxRows      = FOREX_SYMS.map(s => tagged(s, forexPrices, "forex", FOREX_LABELS[s] || s)).filter(Boolean);
...
CRYPTO spot (CoinGecko): ${noteIfEmpty(cryptoCRows, "crypto")}
EQUITIES (Yahoo/FMP): ${noteIfEmpty(equityRows, "equity")}
COMMODITIES: ${noteIfEmpty(metalRows, "commodity")}
FOREX (Yahoo/FMP): ${noteIfEmpty(fxRows, "FX")}`;
  }

  // ── Live QuantBrain signal feed (the actual classified pump/dump events) ─
  // Always include — these are the highest-confidence movers regardless of
  // section filtering above.
  const sigSnap = liveSignals?.length > 0
    ? `\nLIVE QUANTBRAIN SIGNALS (recent pump/dump events): ${liveSignals.slice(0, 5).map(s => `${s.token} ${s.dir}${s.pctMove ? " " + s.pctMove + "%" : ""}`).join(" | ")}`
    : "";
