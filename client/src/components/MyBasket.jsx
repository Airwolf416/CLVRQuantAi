...
export default function MyBasket({ isPro, onUpgrade, storePerps, storeSpot, cryptoPrices, equityPrices, metalPrices }) {
  ...
  const runBasket = useCallback(async () => {
    ...
    // Dynamic data that was previously interpolated into the system prompt
    // (now server-side under feature "basketAI"): the selected trading style,
    // the selected MARKET TYPE rule, and the macro pre-flight status. Sent as
    // DATA-ONLY context; the server fences it as untrusted data.
    const context = `TRADING STYLE: ${styleKey}
${mtRule}
${macroLine}`;
