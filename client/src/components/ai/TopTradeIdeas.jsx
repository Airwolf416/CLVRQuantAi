...
- Respect the leverage caps above` : "";
      ...
      // Server-side system prompt (feature:"tradeIdeas") holds all STATIC
      // instructions. The client sends ONLY dynamic data via `context`:
      // the selected market-type filter, timeframe focus, Kronos eligibility,
      // the mode-specific timeframe block, macro pre-flight rows, Kronos
      // forecast signals, the date/time, and the live market-data sections.
      const context = `TRADE COUNT: up to ${tradeCount} trade ideas
MARKET TYPE FILTER: ${marketTypeFilter}
TIMEFRAME FOCUS: ${tfLabel}
KRONOS ELIGIBILITY: ${isElite ? "enabled (Elite)" : "disabled"}
${todayModeRule ? todayModeRule + "\n" : ""}
