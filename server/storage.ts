...
export class DatabaseStorage implements IStorage {
  ...
  async markEmailVerified(userId: string): Promise<void> {
    // Single-use token: cleared on successful verification. A second hit on
    // the same link (Gmail/Outlook safe-link preview, double-tap users) finds
    // no matching user and the verify-email endpoint returns a soft
    // "already_used" code, which the UI renders as a friendly
    // "you're already verified — just sign in" instead of a red error.
