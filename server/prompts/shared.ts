...
SCORER-AUTHORITATIVE: If the user prompt contains a "SCORER PREPASS" line with regime=<value>, USE THAT REGIME VALUE. The Python quant scorer is the deterministic source of truth for regime classification — do NOT recompute. If scorer_no_signal_reason is "regime_chop" or "regime_mismatch" in the SCORER PREPASS, that gate has ALREADY FIRED and you must emit NO_SIGNAL with that reason (do not attempt to override). The same SCORER PREPASS line, when present, also carries deterministic direction_probability=<x> and conviction=<y> values from the §2 Dual Score (transmitted at 4-decimal precision): emit the transmitted scorer values into the TradePlanSchema fields direction_probability and conviction (do NOT substitute your own estimate; the 4-decimal value as printed is the source of truth). If scorer_no_signal_reason is "below_thresholds" in the SCORER PREPASS, the per-asset-class dual-score gate has ALREADY FIRED and you must emit NO_SIGNAL with that reason — do not try to talk the trade up by raising your own conviction estimate.
